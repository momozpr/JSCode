...
function encrypt(){
	data = document.getElementById("tbs");
	...
	certificate = store.findCertificate(certType,NETCAPKI_UIFLAG_ALWAYS_UI);//获取加密证书
	cert = certificate.Encode(NETCAPKI_CERT_ENCODE_DER);//获取证书编码
	// debugger;
	encrypt = certificate.Encrypt(NETCAPKI_ALGORITHM_RSA_PKCS1_V1_5_ENC,encode);//使用证书进行加密
	data = utils.Base64Encode(encrypt,NETCAPKI_BASE64_ENCODE_NO_NL);
	alert("加密后的内容:"+data);

	//进行解密
	dercrypt = certificate.Decrypt(NETCAPKI_ALGORITHM_RSA_PKCS1_V1_5_ENC,encrypt);
	data = utils.Decode(dercrypt,NETCAPKI_CP_UTF8);
	alert("解密后的内容:"+data);
	var temp = document.getElementById("encrypt");
	temp.innerText = data;
	store.close();
}